 ...
 *
 * PUBLIC ACCESS — the owner's own switch that temporarily drops the code
 * requirement everywhere — does NOT live here. It used to (a Sheet menu item
 * backed by PropertiesService), and was moved to a single static file the
 * client reads directly: `/public-access.json` at the root of this
 * repository. This script has no involvement in it at all now — see
 * `gate.js`'s `publicAccessOn` for the client side of it.
 */
